...
to graphic processors using Vulkan\footnote{
    \url{
    ...
    \url{
        https://ieeexplore.ieee.org/abstract/document/7478342/
    }
}
\@.
These ABIs are not supported by current Java native interop libraries.
LUtils includes multiple ABIs and provides the developer with the ability to implement custom ABIs. However, it has not
...
terms of structure creation and write/read operations.
Thus, answering whether LUtils can be used in performance-critical applications or if other libraries should be preferred.

There have not been many studies analysing performance of Java native interop libraries with respect to structures.
...
The next section provides a concise introduction to LUtils, followed by a description of the experimental methodology.
Subsequently, the results of the experiments are presented and discussed.
