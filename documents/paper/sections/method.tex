Three experiments are performed to compare LUtils to LWJGL, FFMA and JNA\@.
...
More specifically, each experiment features two benchmarks, which are executed separately for each library.
To ensure reproducibility the code and results of each experiment are available on GitHub\footnote{
    \url{
...
This reference benchmark is displayed in the resulting plots.
...
Additionally, Intel SpeedStep, Intel Turbo Boost, hyper-threading and address space layout randomisation have been disabled
...
\begin{description}
    ...
    \item[Wrong stable state:] \hfill \\ To achieve a stable state 50 warmup iterations are included in every benchmark
\end{description}

Each iteration executes the benchmark code repeatedly for one second and every benchmark consists of 50 warmup and 400 measurement iterations, which
...
Except in benchmark 2 of experiment 3, where FFMA exhibits a median variation of 8.29\%.

\subsection{Experiment 1}\label{subsec:experiment-1}

The first experiment evaluates small structures using the four different java native access libraries: LUtils, FFMA, JNA and LWJGL\@.
The structures all have a sizes between 8 bytes and 48 bytes.
Additionally, they contain up to three different element types.
Even though each element can be another structure containing more element types, the size constraint retains a low amount of different
element types.
Notably, the structure with the greatest complexity contains five different element types.
