...
The results of experiment 1 and 2 are presented first, followed by the results of experiment 3.
...
\begin{figure}[htbp]
    ...
    \caption{
        Execution time results of the first benchmark for experiment 1 and 2.
        The benchmarks allocate multiple structures
        without performing write or read operations.
    }
...
\begin{figure}[htbp]
    ...
    \caption{
        Allocation rate results of the first benchmark for experiment 1 and 2.
        The benchmarks allocate multiple structures
        without performing write or read operations.
    }
...
In addition to the main measurement, the figures include a reference benchmark, which allocates the same structures without
...
Nevertheless, both libraries outperform LUtils and JNA in both execution time and allocation rate.
...
FFMA outperforms LUtils and JNA, but displays the largest relative write/read overhead of approximately 80\%
...
benchmark code itself, specifically from Integer, Long and Float wrapper classes that are created during the benchmark.
...
This is likely due to the use of \texttt{VarHandle} instead of written or generated functions, which requires
...
\begin{figure}[htbp]
    ...
    \caption{
        Execution time results of the second benchmark for experiment 1 and 2.
        The benchmarks allocate structures and perform write/read operations on all structure fields.
        Additionally, reference benchmarks are included which allocate the same structures without performing write or read operations,
        to isolate write/read overhead.
    }
...
\begin{figure}[htbp]
    ...
    \caption{
        Allocation rate results of the second benchmark for experiment 1 and 2.
        The benchmarks allocate structures and perform write/read operations on all structure fields.
        Additionally, reference benchmarks are included which allocate the same structures without performing write or read operations,
        to isolate write/read overhead.
    }
...
LWJGL does not initially create the objects required for each array element.
Instead, the objects are only created when required.
...
\begin{figure}[htbp]
    ...
    \caption{
        Execution time results of the first benchmark for experiment 3.
        This benchmark allocates five large structures without performing write or read operations.
        Due to the vast differences between JNA and the other libraries, this graph contains a broken axis. This means
        that the gab between FFMA and JNA is larger than what is visually suggested.
    }
...
\begin{figure}[htbp]
    ...
    \caption{
        Allocation rate results of the first benchmark for experiment 3.
        This benchmark allocates five large structures without performing write or read operations.
        Due to the vast differences between JNA and the other libraries, this graph contains a broken axis. This means
        that the gab between FFMA and JNA is larger than what is visually suggested.
    }
...
In this benchmark the large write and read overhead of LWJGL is not as unexpected as in the write/read benchmarks of the previous experiments.
...
Only JNA creates all of its custom structure classes in advance while creating the parent structure, as mentioned during the interpretation
...
\begin{figure}[htbp]
    ...
    \caption{
        Execution time results of the second benchmark for experiment 3.
        This benchmark allocates a single large structure and performs write/read operations on all structure fields.
        Additionally, a reference benchmark is included, which allocates the same structure without performing write/read operations,
        to isolate write/read overhead.
    }
...
\begin{figure}[htbp]
    ...
    \caption{
        Allocation rate results of the second benchmark for experiment 3.
        This benchmark allocates a single large structure and performs write/read operations on all structure fields.
        Additionally, a reference benchmark is included, which allocates the same structure without performing write/read operations,
        to isolate write/read overhead.
    }
...
Similar to the execution time results, this variability is due to 10 outliers reaching values up to 219,739 bytes.
