...
from classes import SplitInfo, Reference
...
def to_forks_avg(array):
    ...
    timeAvg = {
        ...
        "max": max(array),
        "diff": diff,
        "diff/avg in percent": ((diff / array[0]) * 100)
    }
    return timeAvg
...
class BenchmarkResults:
    ...
    measurementsStd = 0
    measurementsAvg = 0
    measurementsRefAvg = 0
    measurementsMedian = 0
    measurementsRefMedian = 0
    # Allocation Measurements
    allocationsMeasurements = []
    allocationsMeasurementsRef = []
    allocationsMeasurementsStd = 0
    allocationsMeasurementsAvg = 0
    allocationsMeasurementsRefAvg = 0
    allocationsMeasurementsRefMedian = 0
    ...
    def __init__(self, data):
        ...
        self.measurements = []
        self.measurementsRef = []
        self.allocationsMeasurements = []
        self.allocationsMeasurementsRef = []

        for i, item in enumerate(data[0]["primaryMetric"]["rawData"]):
            self.measurements.append(item[0])
            self.measurementsRef.append(item[1])

        for i, item in enumerate(data[0]["secondaryMetrics"]["gc.alloc.rate.norm"]["rawData"]):
            self.allocationsMeasurements.append(item[0])
            self.allocationsMeasurementsRef.append(item[1])
        ...
        self.measurementsRefMedian = np.median(self.measurementsRef)
        self.measurementsRefAvg = np.average(self.measurementsRef)
        self.allocationsMeasurementsRefMedian = np.median(self.allocationsMeasurementsRef)
        self.allocationsMeasurementsRefAvg = np.average(self.allocationsMeasurementsRef)
    ...
    def to_json(self, lwjgl=None):
        data = {
            ...
            "measurementsRefAvg": f"{self.measurementsRefAvg:.10f}",
            "measurementsRefMedian": f"{self.measurementsRefMedian:.10f}",
            ...
            "allocationsMeasurementsRefAvg": f"{self.allocationsMeasurementsRefAvg:.10f}",
            "allocationsMeasurementsRefMedian": f"{self.allocationsMeasurementsRefMedian:.10f}",
            ...
            "diff(avg, exec. time)": f"{(self.measurementsAvg - self.measurementsRefAvg):.10f}; {((self.measurementsAvg - self.measurementsRefAvg)/self.measurementsAvg) * 100:.2f}%",
            "diff(median, exec. time)": f"{(self.measurementsMedian - self.measurementsRefMedian):.10f}; {((self.measurementsMedian - self.measurementsRefMedian)/self.measurementsMedian) * 100:.2f}%",
            "diff(avg, alloc. rate)": f"{(self.allocationsMeasurementsAvg - self.allocationsMeasurementsRefAvg):.10f}; {((self.allocationsMeasurementsAvg - self.allocationsMeasurementsRefAvg)/self.allocationsMeasurementsAvg) * 100:.2f}%",
            "diff(median, alloc. rate)": f"{(self.allocationsMeasurementsMedian - self.allocationsMeasurementsRefMedian):.10f}; {((self.allocationsMeasurementsMedian - self.allocationsMeasurementsRefMedian)/self.allocationsMeasurementsMedian) * 100:.2f}%",
        }
...
def main(outliers):
    ...
    for res in results:
        ...
        summary[res.libName] = res.to_json(lwjgl_res)

        box_plot_reference_data.append(res.measurementsRefAvg)
        bar_graph_reference_data.append(res.allocationsMeasurementsRefMedian)
